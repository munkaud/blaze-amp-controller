class OutputsParser {
  ...
  parse(message) {
    ...
    if (primaryMessage.startsWith('+OUT.COUNT')) {
      ...
      this.instance.log('debug', 'Finished queuing output info commands');
      return true;
    }
    ...
    if (primaryMessage.startsWith('+OUT-')) {
      ...
      this.instance.log('debug', `Parsing output message: ${primaryMessage}`);
      ...
      switch (key) {
        case 'NAME':
          ...
          break;
        case 'SRC':
          this.state.zoneOutputs[outputId] = value;
          this.instance.log('info', `Output ${outputId} assigned to Zone ${value}`);
          this.instance.log('debug', `Updated zoneOutputs: ${JSON.stringify(this.state.zoneOutputs)}`);
          break;
        case 'SRC_CHANNEL':
          if (value === 'S') {
            this.state.outputSrcChannels[outputId] = 'Stereo';
            this.instance.log('info', `Output ${outputId} source channel set to Stereo`);
          } else if (value === 'L') {
            this.state.outputSrcChannels[outputId] = 'Left';
            this.instance.log('info', `Output ${outputId} source channel set to Left`);
          } else if (value === 'R') {
            this.state.outputSrcChannels[outputId] = 'Right';
